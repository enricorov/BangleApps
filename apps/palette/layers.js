...
class Layer {
    ...
    render() {
        ...
        const layer_label_font = 15;

        g.setColor(WHITE).setFont("Vector", layer_label_font).setFontAlign(-1, -1).drawString(this.label.toUpperCase(), 3, 1);
...
var layer_buttons = new Layer("menu");
...
var ok_layout = [
    {
        ...
        size: { w: _screen_mid * 2 - 8, h: _screen_mid - 3 },
        style: TXT_STYLE_DEF,
        label: "Tap away!"
    },
    {
        ...
        size: { w: 40, h: 40 },
        style: { col_bg: RED, col_text:PURPLE, col_border:YELLOW },
        label: ""
    },
]
...
var splash_cb = function (obj, window) {
    ...
    g.setColor(sty.col_bg);
    g.fillRect(0, 0, g.getWidth(), g.getHeight());
    let prev_color = this.get_style();
    this.style.set_style({col_bg:this.style.col_text})
    this.render();
    this.set_style(prev_color);
    g.flip();
